...
public class FLists extends JFrame implements ActionListener, MouseListener,
 ...
 */
		...
		ImageIcon min = new ImageIcon(
				FLists.class.getResource("/image/min.png"));
		ImageIcon exit = new ImageIcon(
				FLists.class.getResource("/image/exit.png"));
		...
		// 设置按扭大小
		button_min.setPreferredSize(new Dimension(min.getIconWidth(), min
				.getIconHeight()));
		button_exit.setPreferredSize(new Dimension(exit.getIconWidth(), exit
				.getIconHeight()));
 ...
 */
		...
		root2 = new IconNode(new ImageIcon(
				FLists.class.getResource("/image/qq1.png")), "我的同学", true);
		root3 = new IconNode(new ImageIcon(
				FLists.class.getResource("/image/qq1.png")), "我的家人", true);
		root4 = new IconNode(new ImageIcon(
				FLists.class.getResource("/image/qq1.png")), "我的老师", true);
		root5 = new IconNode(new ImageIcon(
				FLists.class.getResource("/image/qq1.png")), "特别的", true);
		hash.put(root1, 1);
		hash1.put("1", root1);
		hash.put(root2, 2);
		hash1.put("2", root2);
		...
		if (U.Userdata.getFriend() != null) {
			for (int i = 0; i < U.Userdata.getFriend().size(); i++) {
				if (U.Userdata.getFriend().get(i).getTemp() == 1) {
					map1.put(U.Userdata.getFriend().get(i).getId(), U.Userdata
							.getFriend().get(i));
					hashonline.put(
							U.Userdata.getFriend().get(i).getId(),
							new IconNode(U.Userdata.getFriend().get(i)
									.getHeadicon(), U.Userdata.getFriend()
									.get(i).getId(), true));
				} else {
					map2.put(U.Userdata.getFriend().get(i).getId(), U.Userdata
							.getFriend().get(i));
					hashleaveline.put(
							U.Userdata.getFriend().get(i).getId(),
							new IconNode(U.Userdata.getFriend().get(i)
				...
				if (U.Userdata.getFriend().get(i).getTemp() == 1) {
					...
					case 1:
						if (U.Userdata.getFriend().get(i) != null) {
							root1.add(hashonline.get(U.Userdata.getFriend()
									.get(i).getId()));
							numberingroup[1]++;
						}
						break;
					case 2:
						if (U.Userdata.getFriend().get(i) != null) {
							root2.add(hashonline.get(U.Userdata.getFriend()
									.get(i).getId()));
							numberingroup[2]++;
						}
						break;
					case 3:
						if (U.Userdata.getFriend().get(i) != null) {
							root3.add(hashonline.get(U.Userdata.getFriend()
									.get(i).getId()));
							numberingroup[3]++;
						}
						break;
					case 4:
						if (U.Userdata.getFriend().get(i) != null) {
							root4.add(hashonline.get(U.Userdata.getFriend()
									.get(i).getId()));
							numberingroup[4]++;
						}
						break;
					case 5:
						if (U.Userdata.getFriend().get(i) != null) {
							root5.add(hashonline.get(U.Userdata.getFriend()
									.get(i).getId()));
							numberingroup[5]++;
						}
						break;
			...
			for (int i = 0; i < U.Userdata.getFriend().size(); i++) {
				if (U.Userdata.getFriend().get(i).getTemp() == 0) {
					...
					case 1:
						if (U.Userdata.getFriend().get(i) != null) {
							root1.add(hashleaveline.get(U.Userdata.getFriend()
									.get(i).getId()));
						}
						break;
					case 2:
						if (U.Userdata.getFriend().get(i) != null) {
							root2.add(hashleaveline.get(U.Userdata.getFriend()
									.get(i).getId()));
						}
						break;
					case 3:
						if (U.Userdata.getFriend().get(i) != null) {
							root3.add(hashleaveline.get(U.Userdata.getFriend()
									.get(i).getId()));
						}
						break;
					case 4:
						if (U.Userdata.getFriend().get(i) != null) {
							root4.add(hashleaveline.get(U.Userdata.getFriend()
									.get(i).getId()));
						}
						break;
					case 5:
						if (U.Userdata.getFriend().get(i) != null) {
							root5.add(hashleaveline.get(U.Userdata.getFriend()
									.get(i).getId()));
						}
						break;
					default:
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == remind && vt != null) {
			...
			case 7:
				String group1;
				int res = JOptionPane.showConfirmDialog(null, vt.lastElement()
						.getId() + "想加你为好友，是否同意？");
				if (res == JOptionPane.NO_OPTION
						|| res == JOptionPane.CANCEL_OPTION)
					vt.lastElement().setFlag(9);
				else {
					group1 = JOptionPane.showInputDialog(null, "请输1，2，3，4，5：",
							"选择分组", JOptionPane.QUESTION_MESSAGE);
					vt.lastElement().setFlag(8);
					vt.lastElement().Users.setSign(group1);
				}
				try {
					ObjectOutputStream oos = new ObjectOutputStream(
							ManageThread.Getthreadfrommap(myaccount).s
									.getOutputStream());
			...
			if (!vt.isEmpty()) {
				switch (vt.lastElement().getFlag()) {
				case 2:
					remind.setText("你有新的消息！");
					break;
				case 7:
					remind.setText("你有新的提醒！");
					break;
				}
			} else
				remind.setText("提醒");
			...
			if (node.isLeaf()) {
				...
				System.out.println("1" + myaccount + " " + node.getText()
						+ "/n");
			...
			if (node == root1 || node == root2 || node == root3
					|| node == root4 || node == root5) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == button_min)
			...
			try {
				String input = JOptionPane.showInputDialog(null, "好友帐号",
	...
	public void addperson(MessageType U) {
		...
		f.setdata(U.Users.getId(), U.Users.getName(), U.Users.getSign(),
				U.Users.getHeadicon());
		hashonline.put(f.getId(),
				new IconNode(f.getHeadicon(), f.getId(), true));
	...
	public void onlinefriend(MessageType U) {
		...
		hash1.get(String.valueOf(where)).insert(hashonline.get(U.getId()),
				numberingroup[where]);
		tree.updateUI();
	}
	//好友离线节点向下移操作
